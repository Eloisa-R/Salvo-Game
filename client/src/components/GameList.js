        ...
        return(
            ...
            <ReactTable keyField='id' data={ this.props.games.games } columns={ [{accessor: 'created', Header: 'Created on'}, {accessor: 'gamePlayers[0].player.email', Header: 'Player 1'},
                {accessor: 'gamePlayers[1].player.email', Header: 'Player 2'}, {accessor: 'gamePlayers', Header: 'Actions',
                    Cell: button => (
                        this.props.games.player.id === "null"?<div>Login required</div>: button.value[1]? (button.value[0].player.id === this.props.games.player.id? <a href={`/shiplocations/${button.value[0].id}`}>Continue game</a>: button.value[1].player.id === this.props.games.player.id?<a href={`/shiplocations/${button.value[1].id}`}>Continue game</a>:"") : button.value[0].player.id === this.props.games.player.id? (<a href={`/shiplocations/${button.value[0].id}`}>Start game</a>) :(<button>Join game</button>)
                    )
